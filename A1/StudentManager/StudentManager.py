...
def updateStudentWindow():
    ...
    Label(updateWin, text="Course Mark 1 (0-20):").pack()
    c1_entry = Entry(updateWin)
    c1_entry.insert(0, str(student["c1"]))
    c1_entry.pack()

    Label(updateWin, text="Course Mark 2 (0-20):").pack()
    c2_entry = Entry(updateWin)
    c2_entry.insert(0, str(student["c2"]))
    c2_entry.pack()

    Label(updateWin, text="Course Mark 3 (0-20):").pack()
    ...
    def saveUpdates():
        ...
        if not (0 <= new_c1 <= 20 and 0 <= new_c2 <= 20 and 0 <= new_c3 <= 20):
            messagebox.showerror("Invalid Course Mark", "Coursework marks must be 0–20.")
            return
    ...
    Button(updateWin, text="Save Changes", command=saveUpdates).pack(pady=10)
...
def deleteStudentRecord():
    ...
    with open("StudentManager/studentMarks.txt", "w") as f:
        for line in lines:
            ...
            if len(parts) == 6 and parts[0] == student_id:
                continue  # skip matching student ID (delete)
            f.write(line if line.endswith("\n") else line + "\n")

    # Reload in-memory list after deletion
    global students
...
def addStudentWindow():
    ...
    submitButton=Button(win, text="Submit", command=submitNewStudent).pack(pady=10)
...
BSULabel = Label(root, image = BSUPhoto,bg="#B5C2FF")
BSULabel.image = BSUPhoto
BSULabel.pack(pady= 5)
Name = Label(root, text=" Student Manager", font=('Roboto',25), fg='#22263d',bg="#B5C2FF")
Name.pack(pady=5)
...
#This was originally supposed to be like how it was shown in the first "read .txt file" exercise but I had to utilize treeview to properly show the data in table format

btnFrame = Frame(root,bg="#B5C2FF")
btnFrame.pack()



viewButton = Button(btnFrame, text="View All Students",bg = "#fc1e34", fg= "white", height=5,width=20, command=viewAll).grid(row=0, column=0, padx=5, pady=5)
highestMarkButton = Button(btnFrame, text="Highest Mark",bg = "#fc1e34", fg= "white",height=5, width=20, command=viewHighest).grid(row=0, column=1, padx=5, pady=5)
lowestMarkButton = Button(btnFrame, text="Lowest Mark",bg = "#fc1e34", fg= "white",height=5, width=20, command=viewLowest).grid(row=0, column=2, padx=5, pady=5)
ascendingButton = Button(btnFrame, text="Sort Ascending",bg = "#fc1e34", fg= "white", height=5,width=20, command=sortAscending).grid(row=1, column=1, padx=5, pady=5)
descendingButton = Button(btnFrame, text="Sort Descending",bg = "#fc1e34", fg= "white",height=5, width=20, command=sortDescending).grid(row=1, column=2, padx=5, pady=5)
studentButton = Button(btnFrame, text="Add Student",bg = "#fc1e34", fg= "white",height=5, width=20, command=addStudentWindow).grid(row=1, column=0, padx=5, pady=5)
deleteButton = Button(btnFrame, text="Delete Student Record",bg = "#fc1e34", fg= "white",height=5,width=20, command=deleteStudentRecord).grid(row=2, column=0, padx=5, pady=5)
updateButton = Button(btnFrame, text="Update Student Record",bg = "#fc1e34", fg= "white",height=5,width = 20, command=updateStudentWindow).grid(row=2, column=1, padx=5, pady=5) 

quitButton = Button(root, text="Quit Student Manager",bg = "#fc1e34", fg= "white", command=quitButton).pack(pady=10)
...
summaryLabel = Label(root, text="", font=("Arial", 12),bg="#B5C2FF")
